...
async def temp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_allowed(update.effective_user.id, ALLOWED_IDS):
        return
    try:
        cpu_temp = SystemModel.get_cpu_temp()
        await update.message.reply_text(
            f"<b>🌡️ CPU TEMP</b>\n<pre>{cpu_temp}</pre>",
            parse_mode="HTML",
        )
    except Exception as e:
        await update.message.reply_text(f"❌ Error Temp: {e}")
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg = render_status(cpu, ram_data, disk_data, temp_value)
...
async def list_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not files:
            await update.message.reply_text(
                f"📂 <b>Folder kosong, Bos.</b>\n💾 <b>Sisa Disk:</b> {format_size(free)}",
                parse_mode="HTML",
            )
            return

        file_list = [
            f"• <code>{f.name}</code>\n  └─ 📦 <b>{format_size(f.stat().st_size)}</b>"
            for f in files[:15]
        ]
        output = (
    ...
    try:
        ...
        if data.startswith("yts_redir_"):
            ...
            await query.edit_message_text(
                "🚀 <b>Ok Bos!</b> Mengalihkan download ke server (Full HD)...",
                parse_mode="HTML",
            )
            ...
            await process.wait()
            await query.message.reply_text("✅ <b>Selesai!</b> Video sudah ada di server.\nCek via /list.", parse_mode="HTML")
            ...
            if idx < len(files):
                ...
                kb = [[
                    InlineKeyboardButton("✅ Ya", callback_data=f"confdel_{idx}"),
                    InlineKeyboardButton("❌ Batal", callback_data="cancel_cleanup"),
                ]]
